...
class CustomTopBar(QWidget):
    def __init__(self, parent, stacked_widget):
        ...
        self.draggable = True  # Enable dragging functionality
    ...
    def mousePressEvent(self, event):
        if self.draggable and event.button() == Qt.LeftButton:
            self.offset = event.pos()  # Record the initial position on press

    def mouseMoveEvent(self, event):
        if self.draggable and self.offset is not None:
            new_pos = self.parent().pos() + (event.globalPos() - self.offset)
            self.parent().move(new_pos)  # Move the parent window
            self.offset = event.globalPos()  # Update the offset
