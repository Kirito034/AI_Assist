    ...
    for command in commands:
        if command.startswith("open "):  # Handle "open" commands.
            ...
            if "open file" == command:
                ...
                func = asyncio.to_thread(appopen, command.removeprefix("open "))  # Schedule app opening.
                funcs.append(func)
            ...
            print(f"No Function Found for command: {command}")  # Print an error for unrecognized commands.

    results = await asyncio.gather(*funcs)  # Execute all tasks concurrently.
    for result in results:  # Process the results.
        yield result  # Yield each result.
